  ...
  draw() {
    ...
    for (let i = 0, len = this._bbOffsets.length; i < len; i++) {
      ...
      if (visibleRange) {
        ...
        canvas.drawElement({start: visibleRange.start, stop: visibleRange.stop, centerOffset, color: this[bbOffset.type].color.rgbaString, width: this[bbOffset.type].adjustedThickness, showShading: false, showBorder: false});
      }
    }
  }
