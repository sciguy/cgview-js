 ...
 * [backgroundColor](#backgroundColor) | String   | Color of the rectangle or text halos and swatch outlines, including opacity. Omit or set to undefined to inherit the current map background color. See {@link Color} for details.
 * [backgroundStyle](#backgroundStyle) | String   | Background presentation: *rect* or *halo* [Default: 'rect']. Halo stroke width is one quarter of each item's font size, with matching outlines outside the swatches.
 ...
 *
 * ```js
 * cgv.legend.update({backgroundStyle: 'halo'});
 * // Use an explicit color for the text halos and swatch outlines.
 * cgv.legend.update({backgroundColor: 'rgba(255,255,255,0.8)'});
 * // Resume following the map background color, including future changes.
 * cgv.legend.update({backgroundColor: undefined});
 * ```
 *
 * @extends CGObject
 */
class Legend extends CGObject {
   ...
   * or *halo*. Both use backgroundColor, including its opacity. Text halo stroke
   * width is one quarter of each item's font size. Swatch outlines extend the
   * same distance outside their fills, preserving swatch colors and hit bounds.
   */
  get backgroundStyle() {
  ...
  update(attributes) {
    this.viewer.updateRecords(this, attributes, {
      recordClass: 'Legend',
      validKeys: ['on', 'position', 'anchor', 'defaultFont', 'defaultFontColor', 'defaultDecoration', 'defaultMinArcLength', 'textAlignment', 'backgroundColor', 'backgroundStyle', 'visible']
    });
   ...
   * @param {Boolean} [redraw=true] - Repaint shared layers after measuring.
   * @private
   */
  refresh(redraw = true) {
    const box = this.box;
  ...
  /**
   * Fill rectangular backgrounds. Halo backgrounds are painted with the items.
   * @private
   */
  fillBackground() {
  ...
  /**
   * Invert item colors, the default font color, and any explicit background.
   * Inherited backgrounds continue following the map, which is inverted
   * separately by the viewer.
   */
  invertColors(all = true) {
    const attributes = {defaultFontColor: this.defaultFontColor.invert().rgbaString};
  ...
  /**
   * Draw the legend, repainting its shared layers to preserve overlapping text
   * and avoid accumulating translucent halos or outlines on repeated draws.
   * @private
   */
  draw() {
  ...
  _draw() {
    ...
    ctx.save();
    this.fillBackground();
    const haloColor = this.backgroundStyle === 'halo' ? this.backgroundColor.rgbaString : undefined;
    ctx.lineWidth = 1;
    ctx.lineJoin = 'miter';
    ctx.setLineDash([]);
    ...
    for (let i = 0, len = this._items.length; i < len; i++) {
      ...
      const haloWidth = legendItem.font.size * HALO_WIDTH_RATIO;
      ctx.font = legendItem.font.css;
      ctx.textAlign = legendItem.textAlignment;
      if (drawSwatch) {
        const swatchX = legendItem.swatchX();
        if (haloColor) {
          // Match the visible half of the text stroke without painting beneath
          // translucent swatch fills. The stroke's inner edge meets the fill.
          const outlineWidth = haloWidth / 2;
          const offset = outlineWidth / 2;
          ctx.lineWidth = outlineWidth;
          ctx.strokeStyle = haloColor;
          ctx.strokeRect(swatchX - offset, y - offset, swatchWidth + outlineWidth, swatchWidth + outlineWidth);
        }
        // Draw Swatch
        ...
        if (legendItem.swatchSelected || legendItem.swatchHighlighted) {
          const border = 2;
          ctx.lineWidth = 1;
          ctx.strokeStyle = legendItem.swatchSelected ? 'black' : 'grey';
      ...
      this.canvas.drawText(ctx, legendItem.name, legendItem.textX(), y + legendItem.height - 1, {
        haloColor,
        haloWidth,
      });
    }
